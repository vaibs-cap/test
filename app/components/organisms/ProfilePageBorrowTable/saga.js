...
import { fetchUserBorrowedBooksSuccess, fetchUserBorrowedBooksFailure, returnUserBorrowedBooksSuccess, returnUserBorrowedBooksFailure } from './actions';
...
export function* returnBookRequests(action) {
  try {
    ...
    const res = yield call(Api.returnBook, action.payload.userId, action.payload.bookId);
    console.log(res);
    if (res?.status==200) {
