...
const ProfilePageNewRequestTable = ({className}) => {
  ...
  const columns = [
    ...
    {
      render: (text, record) => (
        <CapButton type="secondary" size="small" variant="contained" onClick={()=>showAcceptModal(text.request_id)}>
          Approve
        </CapButton>
      ),
      width: '7%',
    ...
    {
      render: (text, record) => (
        <CapButton className="request-cancel-btn" type="secondary" size="small" variant="contained"  onClick={()=>showCancelModal(text.request_id)}>
          Cancel
        </CapButton>
      ),
      width: '7%',
    ...
    <CapRow className={className}>
      ...
      <CapRow className="modal">
        <CapModal
          ...
          width={700}
          >
          <CapInput value={newReq.book_name} name='book_name' label="Title" onChange={handleChange}></CapInput>
          <CapInput value={newReq.book_author} name='book_author' label="Author" onChange={handleChange}></CapInput>
          <CapInput name="book_genre" label="Genre" onChange={handleChange}></CapInput>
          <CapInput name="book_description" label="Description" onChange={handleChange}></CapInput>
          <CapDatePicker name="anticipated date" label="Ancipated Date" onChange={handleDate}/>
        </CapModal>
        ...
        <CapModal
          ...
          width={700}
          >
          <CapInput name='reason' label="Reason" onChange={handleCancelChange}></CapInput>
        </CapModal>
